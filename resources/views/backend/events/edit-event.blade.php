                        ...
                        <form class="forms-sample" id="eventForm">
                            ...
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control tinymceExample" name="event_description" id="event_description" rows="10"
                                        value="">{{ $event->event_description }}</textarea>
                                </div>
                            </div>
                            <hr>
                            <div class="row mb-4">
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" name="location_held" id="location_held" rows="5" value="">{{ $event->location_held }}</textarea>
                                </div>
                            </div>
                            <hr>
                            <div class="mb-2">
                                ...
                                <div class="ticket_wrapper">
                                    <div class="row">
                                        @foreach ($event->ticketCategory as $item)
                                            <div class="row" id="row-{{ $item->ticket_category_id }}">
                                                <input type="hidden" name="ticket_category_id[]" id="ticket_category_id"
                                                    value="{{ $item->ticket_category_id }}">
                                                <div class="col-md-4">
                                                    <div class="input-group mb-3">
                                                        <span class="input-group-text" id="basic-addon2">Category
                                                            Name</span>
                                                        <input type="text" id="ticket_category_name"
                                                            class="form-control" name="ticket_category_name[]"
                                                            value="{{ $item->ticket_category_name }}" />
                                                    </div>
                                                </div>
                                                <div class="col-md-3">
                                                    <div class="input-group mb-3">
                                                        <span class="input-group-text" id="basic-addon2">Ticket
                                                            Pricing</span>
                                                        <input type="number" id="ticket_category_price"
                                                            class="form-control" name="ticket_category_price[]"
                                                            value="{{ $item->ticket_category_price }}" />
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="input-group mb-3">
                                                        <span class="input-group-text" id="basic-addon2">Ticket
                                                            Quota</span>
                                                        <input type="number" id="ticket_category_quota"
                                                            class="form-control" name="ticket_category_quota[]"
                                                            value="{{ $item->ticket_category_quota }}" />
                                                    </div>
                                                </div>
                                                <div class="col-md-2">
                                                    <div class="input-group mb-3">
                                                        <span class="input-group-text" id="basic-addon2">Status</span>
                                                        <select class="form-select" name="ticket_category_status[]" id="ticket_category_status">
                                                            <option value="available" {{ $item->ticket_category_status == 'available' ? 'selected' : '' }}>Available</option>
                                                            <option value="sold out" {{ $item->ticket_category_status == 'sold out' ? 'selected' : '' }}>Sold Out</option>
                                                            <option value="soon" {{ $item->ticket_category_status == 'soon' ? 'selected' : '' }}>Soon</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-1">
                                                    <button type="button" id="remove-ticket"
                                                        data-id="{{ $item->ticket_category_id }}"
                                                        class="btn btn-danger"><i class="mdi mdi-minus"></i></button>
                                                </div>
                                            </div>
                                        @endforeach
                            ...
                            <div class="row mb-3 text-center align-items-center">
                                <div class="col-md-12">
                                    <button class="btn btn-primary w-25" type="submit" id="submitBtnEvent">Save</button>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '#remove-ticket', function() {
                var ticket_category_id = $(this).attr('data-id');
                ...
                swalWithBootstrapButtons
                    ...
                    .then((result) => {
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: "{{ route('ticketCategory.destroy') }}",
                                data: {
